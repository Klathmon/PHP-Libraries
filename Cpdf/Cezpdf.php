...
class Cezpdf extends Cpdf
{
    ...
    function ezGetMargin($which)
    {
        ...
        case 't':
        case 'top':
            return $this->ez['topMargin'];
        case 'tc':
        case 'topc':
            return $this->ez['pageHeight'] - $this->ez['topMargin'];
        case 'l':
        case 'left':
        ...
        case 'rc':
        case 'rightc':
            return $this->ez['pageWidth'] - $this->ez['rightMargin'];
        case 'b':
        case 'bottom':
            return $this->ez['bottomMargin'];
        default :
            return 0;
        }
    }
    ...
    function ezColumnsStart($options = array())
    {
        ...
        $width                        = ($this->ez['pageWidth'] - $this->ez['leftMargin'] - $this->ez['rightMargin'] -
                ($options['num'] - 1) * $options['gap']) / $options['num'];
    ...
    function ezNewPage()
    {
        ...
        if (isset ($this->ez['columns']) && $this->ez['columns']['on'] == 1) {
            ...
            $this->ez['leftMargin']  = $this->ez['columns']['margins'][0] +
                ($this->ez['columns']['colNum'] - 1) * ($this->ez['columns']['options']['gap'] + $width);
            $this->ez['rightMargin'] = $this->ez['pageWidth'] - $this->ez['leftMargin'] - $width;
        }
        ...
        if ($pageRequired) {
            ...
            if (isset ($this->ez['insertMode']) && $this->ez['insertMode'] == 1) {
                $id = $this->ezPages[$this->ezPageCount] = $this->newPage(
                    1, $this->ez['insertOptions']['id'], $this->ez['insertOptions']['pos']
                );
     ...
     */
    function ezPrvtTableColumnHeadings(
        $cols, $pos, $maxWidth, $height, $decender, $gap, $size, & $y, $optionsAll = array()
    )
    {
    ...
    function ezTable(& $data, $cols = '', $title = '', $options = '')
    {
        ...
        while ($abortTable) {
            ...
            if ($options['showHeadings']) {
                ...
                $headingHeight = $this->ezPrvtTableColumnHeadings(
                    $cols, $pos, $maxWidth, $height, $decender, $options['rowGap'], $options['fontSize'], $y, $options
                );
                $y0            = $y + $headingHeight;
                $y1            = $y;
                    ...
                    while (!$abortTable && ($newPage || $newRow)) {
                        ...
                        if ($newPage || $y < $this->ez['bottomMargin']
                            ...
                            if ($options['showLines']) {
                                ...
                                $this->ezPrvtTableDrawLines(
                                    $pos, $options['gap'], $x0, $x1, $y0, $y1, $y2, $options['lineCol'],
                                ...
                                while (strlen($line) || $start) {
                                    ...
                                    if ($colNewPage) {
                                        ...
                                        $line = $this->addTextWrap(
                                            $pos[$colName], $this->y, $maxWidth[$colName], $options['fontSize'], $line,
                                            $just
                                        );
                                    }
                                }
                        ...
                        if ($options['shaded'] && $cnt % 2 == 0) {
                            $this->closeObject();
                            $this->setColor(
                                $options['shadeCol'][0], $options['shadeCol'][1], $options['shadeCol'][2], 1
                            );
                            $this->filledRectangle(
                                $x0 - $options['gap'] / 2, $y + $decender + $height - $mx, $x1 - $x0, $mx
                            );
                            $this->reopenObject($textObjectId);
                        }
                        ...
                        if ($options['shaded'] == 2 && $cnt % 2 == 1) {
                            $this->closeObject();
                            $this->setColor(
                                $options['shadeCol2'][0], $options['shadeCol2'][1], $options['shadeCol2'][2], 1
                            );
                            $this->filledRectangle(
                                $x0 - $options['gap'] / 2, $y + $decender + $height - $mx, $x1 - $x0, $mx
                            );
                        ...
                        if ($options['showLines'] > 1) {
                            ...
                            $this->saveState();
                            $this->setStrokeColor(
                                $options['lineCol'][0], $options['lineCol'][1], $options['lineCol'][2], 1
                            );
                            ...
                            $this->line(
                                $x0 - $options['gap'] / 2, $y + $decender + $height, $x1 - $options['gap'] / 2,
                                $y + $decender + $height
                            );
                            $this->restoreState();
                            //        $this->reopenObject($textObjectId);
        ...
        if ($options['showLines']) {
            ...
            $this->ezPrvtTableDrawLines(
                $pos, $options['gap'], $x0, $x1, $y0, $y1, $y2, $options['lineCol'], $options['innerLineThickness'],
                $options['outerLineThickness'], $options['showLines']
            );
        }
    ...
    function ezText($text, $size = 0, $options = array(), $test = 0)
    {
        ...
        if (is_array($options) && isset ($options['aright'])) {
            ...
            $right
                = $this->ez['pageWidth'] - $this->ez['rightMargin'] - ((is_array($options) && isset ($options['right']))
                    ? $options['right'] : 0);
        }
        if ($size <= 0) {
        ...
        foreach ($lines as $line) {
            ...
            while (strlen($line) || $start) {
                ...
                if (is_array($options) && isset ($options['aleft'])) {
                    ...
                    $left
                        = $this->ez['leftMargin'] + ((is_array($options) && isset ($options['left'])) ? $options['left']
                            : 0);
                }
                if (is_array($options) && isset ($options['aright'])) {
                    $right = $options['aright'];
    ...
    function ezImage($image, $pad = 5, $width = 0, $resize = 'full', $just = 'center', $border = '')
    {
        ...
        if ($type == "jpeg") {
            $this->addJpegFromFile(
                $image, $this->ez['leftMargin'] + $pad + $offset,
                $this->y + $this->getFontHeight($this->ez['fontSize']) - $pad - $height, $width
            );
        }
        ...
        if ($border != '') {
            ...
            $this->rectangle(
                ...
                $this->y + $this->getFontHeight($this->ez['fontSize']) - $pad - $height, $width, $height
            );
    ...
    function alink($info, $internal = 0)
    {
        ...
        case 'eol' :
            ...
            // add underlining
            if ($internal) {
                $this->addInternalLink(
                    $start['url'], $start['x'], $start['y'] + $start['decender'], $info['x'],
