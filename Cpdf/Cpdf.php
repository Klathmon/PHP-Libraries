...
class Cpdf
{
            ...
            if (count($o['info']['pages'])) {
                ...
                if ((isset($o['info']['fonts']) && count($o['info']['fonts'])) || isset($o['info']['procset'])) {
                    ...
                    if (isset($o['info']['mediaBox'])) {
                        ...
                        $res
                            .=
                            "\n/MediaBox [" . sprintf('%.3f', $tmp[0]) . ' ' . sprintf('%.3f', $tmp[1]) . ' ' . sprintf(
                                '%.3f', $tmp[2]
                            ) . ' ' . sprintf('%.3f', $tmp[3]) . ']';
                    }
                }
    ...
    function o_font($id, $action, $options = '')
    {
        ...
        case 'new':
            $this->objects[$id]                    = array(
                't' => 'font', 'info' => array('name' => $options['name'], 'SubType' => 'Type1')
            );
    ...
    function o_info($id, $action, $options = '')
    {
        ...
        case 'new':
            ...
            $this->objects[$id] = array(
                't' => 'info', 'info' => array(
                    'Creator' => 'R and OS php pdf writer, http://www.ros.co.nz', 'CreationDate' => $date
                )
            );
            break;
        case 'Title':
    ...
    function o_page($id, $action, $options = '')
    {
        ...
        case 'new':
            $this->numPages++;
            $this->objects[$id] = array(
                't' => 'page', 'info' => array('parent' => $this->currentNode, 'pageNum' => $this->numPages)
            );
            if (is_array($options)) {
    ...
    function o_image($id, $action, $options = '')
    {
        ...
        case 'new':
            // make the new object
            $this->objects[$id]                    = array(
                't' => 'image', 'data' => $options['data'], 'info' => array()
            );
                    ...
                    if (strlen($options['pdata'])) {
                        ...
                        if (isset($options['transparency'])) {
                            ...
                            case 'indexed':
                                $tmp
                                                                    =
                                    ' [ ' . $options['transparency']['data'] . ' ' . $options['transparency']['data']
                                    . '] ';
    ...
    function o_encryption($id, $action, $options = '')
    {
        ...
        case 'new':
            ...
            $pad = chr(0x28) . chr(0xBF) . chr(0x4E) . chr(0x5E) . chr(0x4E) . chr(0x75) . chr(0x8A) . chr(0x41) . chr(
                    0x64
                ) . chr(0x00) . chr(0x4E) . chr(0x56) . chr(0xFF) . chr(0xFA) . chr(0x01) . chr(0x08) . chr(0x2E) . chr(
                    0x2E
                ) . chr(0x00) . chr(0xB6) . chr(0xD0) . chr(0x68) . chr(0x3E) . chr(0x80) . chr(0x2F) . chr(0x0C) . chr(
                    0xA9
    ...
    function encryptInit($id)
    {
        ...
        $tmp
            .=
            chr(hexdec(substr($hex, 4, 2))) . chr(hexdec(substr($hex, 2, 2))) . chr(hexdec(substr($hex, 0, 2))) . chr(
                0
            ) . chr(0);
        $key = $this->md5_16($tmp);
        ...
        if (!isset($this->fonts[$fontName])) {
            ...
            if (isset($this->fonts[$fontName])) {
                ...
                if (substr($fontName, -4) == '.afm' && strlen($fbtype)) {
                    ...
                    $list  = array(
                        'Ascent'   => 'Ascender', 'CapHeight' => 'CapHeight', 'Descent' => 'Descender',
                    ...
                    if ($fbtype == 'pfb') {
                        ...
                        $this->o_contents(
                            $this->numObj, 'add', array('Length1' => $l1, 'Length2' => $l2, 'Length3' => $l3)
                        );
                    } else {
                        if ($fbtype == 'ttf') {
                            $l1 = strlen($data);
    ...
    {
        $this->objects[$this->currentContents]['c']
            .= "\n" . sprintf('%.3f', $x1) . ' ' . sprintf('%.3f', $y1) . ' m ' . sprintf('%.3f', $x2) . ' ' . sprintf(
                '%.3f', $y2
            ) . ' l S';
    }
    ...
    function curve($x0, $y0, $x1, $y1, $x2, $y2, $x3, $y3)
    {
        ...
        $this->objects[$this->currentContents]['c']
            .= "\n" . sprintf('%.3f', $x0) . ' ' . sprintf('%.3f', $y0) . ' m ' . sprintf('%.3f', $x1) . ' ' . sprintf(
                '%.3f', $y1
            );
        $this->objects[$this->currentContents]['c']
            .= ' ' . sprintf('%.3f', $x2) . ' ' . sprintf('%.3f', $y2) . ' ' . sprintf('%.3f', $x3) . ' ' . sprintf(
                '%.3f', $y3
            ) . ' c S';
    }
    ...
    function ellipse($x0, $y0, $r1, $r2 = 0, $angle = 0, $nSeg = 8, $astart = 0, $afinish = 360, $close = 1, $fill = 0)
    {
        ...
        if ($angle != 0) {
            ...
            $tmp = "\n q ";
            $tmp .= sprintf('%.3f', cos($a)) . ' ' . sprintf('%.3f', (-1.0 * sin($a))) . ' ' . sprintf('%.3f', sin($a))
                . ' ' . sprintf('%.3f', cos($a)) . ' ';
        ...
        for ($i = 1; $i <= $nSeg; $i++) {
            ...
            $this->objects[$this->currentContents]['c']
                .=
                ' ' . sprintf('%.3f', ($a1 - $c1 * $dtm)) . ' ' . sprintf('%.3f', ($b1 - $d1 * $dtm)) . ' ' . sprintf(
                    '%.3f', $a1
                ) . ' ' . sprintf('%.3f', $b1) . ' c';
            $a0 = $a1;
            $b0 = $b1;
    ...
    function polygon($p, $np, $f = 0)
    {
        ...
        for ($i = 2; $i < $np * 2; $i = $i + 2) {
            $this->objects[$this->currentContents]['c']
                .= sprintf('%.3f', $p[$i]) . ' ' . sprintf('%.3f', $p[$i + 1]) . ' l ';
        }
        if ($f == 1) {
            $this->objects[$this->currentContents]['c'] .= ' f';
        } else {
    ...
    {
        $this->objects[$this->currentContents]['c']
            .= "\n" . sprintf('%.3f', $x1) . ' ' . sprintf('%.3f', $y1) . ' ' . sprintf('%.3f', $width) . ' ' . sprintf(
                '%.3f', $height
            ) . ' re f';
    }
    ...
    function newPage($insert = 0, $id = 0, $pos = 'after')
    {
        ...
        if ($this->currentStrokeColour['r'] >= 0) {
            $this->setStrokeColor(
                $this->currentStrokeColour['r'], $this->currentStrokeColour['g'], $this->currentStrokeColour['b'], 1
            );
        }
     ...
     * @access private
     */
    protected function PRVTcheckTextDirective1(
        &$text, $i, &$f, $final, &$x, &$y, $size = 0, $angle = 0, $wordSpaceAdjust = 0
    )
    {
        $directive = 0;
        $j         = $i;
                    ...
                    if ($text[$j] == '>') {
                        ...
                        if ($p !== false) {
                            ...
                            $this->currentTextState
                                = substr($this->currentTextState, 0, $p) . substr($this->currentTextState, $p + 1);
                        }
                        $directive = $j - $i + 1;
                    }
                    break;
                    ...
                    if ($k !== false && $text[$j] == ':') {
                        ...
                        if (!isset($func) || !strlen(trim($func))) {
                            ...
                            if ($final) {
                                ...
                                $info = array(
                                    'x'         => $tmp[0], 'y' => $tmp[1], 'angle' => $angle, 'status' => 'end',
                                    'p'         => $parm, 'nCallback' => $this->nCallback
                                );
                                $x    = $tmp[0];
                                $y    = $tmp[1];
                                $ret  = $this->$func($info);
                ...
                if ($k !== false && $text[$j] == ':') {
                    ...
                    if (!isset($func) || !strlen(trim($func))) {
                        ...
                        if ($final) {
                            ...
                            $info = array(
                                'x' => $tmp[0], 'y' => $tmp[1], 'angle' => $angle, 'status' => 'start', 'p' => $parm,
                                'f' => $func, 'height' => $this->getFontHeight($size),
    ...
    function addText($x, $y, $size, $text, $angle = 0, $wordSpaceAdjust = 0)
    {
        ...
        if ($this->nCallback > 0) {
            for ($i = $this->nCallback; $i > 0; $i--) {
                // call each function
                $info = array(
                    'x'         => $x, 'y' => $y, 'angle' => $angle, 'status' => 'sol', 'p' => $this->callback[$i]['p'],
                    'nCallback' => $this->callback[$i]['nCallback'], 'height' => $this->callback[$i]['height'],
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($directive) {
                ...
                if ($i > $start) {
                    ...
                    $this->objects[$this->currentContents]['c']
                        .= ' /F' . $this->currentFontNum . ' ' . sprintf('%.1f', $size) . ' Tf ';
                ...
                if ($f) {
                    ...
                    if ($angle == 0) {
                        ...
                        $a   = deg2rad((float)$angle);
                        $tmp = "\n" . 'BT ';
                        $tmp .= sprintf('%.3f', cos($a)) . ' ' . sprintf('%.3f', (-1.0 * sin($a))) . ' ' . sprintf(
                                '%.3f', sin($a)
                            ) . ' ' . sprintf('%.3f', cos($a)) . ' ';
        ...
        if ($this->nCallback > 0) {
            for ($i = $this->nCallback; $i > 0; $i--) {
                ...
                $info = array(
                    'x'      => $tmp[0], 'y' => $tmp[1], 'angle' => $angle, 'status' => 'eol',
    ...
    function addPngFromFile($file, $x, $y, $w = 0, $h = 0)
    {
        ...
        $this->objects[$this->currentContents]['c'] .= "\nq";
        $this->objects[$this->currentContents]['c']
            .= "\n" . sprintf('%.3f', $w) . " 0 0 " . sprintf('%.3f', $h) . " " . sprintf('%.3f', $x) . " " . sprintf(
                '%.3f', $y
            ) . " cm";
        $this->objects[$this->currentContents]['c'] .= "\n/" . $label . ' Do';
        $this->objects[$this->currentContents]['c'] .= "\nQ";
