...
function get($file, $silent, $nobackup, $basePath)
{
    if (ini_get('allow_url_fopen')) {
        if ($data = @file_get_contents($file)) {
            if (!$silent) {
                print "loading and converting YAML data...\n";
            };
            ...
            if (file_exists($basePath . "resources/regexes.json")) {
                if (!$nobackup) {
                    if (!$silent) {
                        print("backing up old JSON file...\n");
                    }
                    if (!copy(
                        $basePath . "resources/regexes.json",
                        $basePath . "resources/regexes." . date("Ymdhis") . ".json"
                    )
                    ) {
                        if (!$silent) {
        ...
        if (!$silent) {
            print("ERROR: the allow_url_fopen option is not enabled in php.ini. it needs to be set to 'On' for this feature to work...\n");
        }
    }
}
...
if (php_sapi_name() == 'cli') {
    ...
    if (isset($args["g"])) {
            ...
            while (($line = fgets($data)) !== false) {
                ...
                if (!empty($ua) && ($ua != "-")) {
                    ...
                    if (($result->ua->family == "Other") && ($result->device->family != "Spider")) {
                        ...
                        if (($result->os->family == "Other") && ($result->device->family != "Spider")) {
                            $output = "OS Not Found: " . $ua . "  [" . $line . "]\n";
                            $show   = "O";
                        } else {
                            if ($result->device->family == "Generic Smartphone") {
                                $output = "GS:           " . $ua . "  [" . $line . "]\n";
                                $show   = "GS";
                            } else {
                                if ($result->device->family == "Generic Feature Phone") {
                                    $output = "GFP:          " . $ua . "  [" . $line . "]\n";
                                    $show   = "GFP";
                                }
                            }
                    ...
                    if ((($show == "U") || ($show == "O") || ($show == "GS") || ($show == "GFP"))
                        && !in_array(
                            $ua, $saved
                        )
                    ) {
        ...
        if (isset($args["j"]) && $args["j"]) {
            ...
            if (isset($argv[1])
                && (($argv[1] != "-j") && ($argv[1] != "-p") && ($argv[1] != "-l") && ($argv[1] != "-s")
                    && ($argv[1] != "-n"))
            ) {
