...
class UAParser
{
    ...
    public function __construct($customRegexesFile = null)
    {
        ...
        $regexesFile = ($customRegexesFile !== null) ? $customRegexesFile
            : dirname(__FILE__) . DIRECTORY_SEPARATOR . 'resources/regexes.json';
        if (file_exists($regexesFile)) {
            ...
            if ($customRegexesFile !== null) {
                $message      = 'ua-parser can\'t find the custom regexes file you supplied (' . $customRegexesFile
                    . '). Please make sure you have the correct path.';
                $instruction1 = '';
                $instruction2 = '';
            } else {
                $message
                              = 'Please download the regexes.json file before using uaparser.php. You can type the following at the command line to download the latest version: ';
    ...
    public function uaParse($uaString = '')
    {
        ...
        foreach ($uaRegexes as $uaRegex) {
            ...
            if (preg_match(
                '/' . str_replace('/', '\/', str_replace('\/', '/', $uaRegex->regex)) . '/i', $uaString, $matches
            )
            ) {
                ...
                // ua name
                $ua->family = isset($uaRegex->family_replacement) ? str_replace(
    ...
    public function osParse($uaString = '')
    {
        ...
        foreach ($osRegexes as $osRegex) {
            ...
            if (preg_match(
                '/' . str_replace('/', '\/', str_replace('\/', '/', $osRegex->regex)) . '/i', $uaString, $matches
            )
            ) {
    ...
    public function deviceParse($uaString = '')
    {
        ...
        foreach ($deviceRegexes as $deviceRegex) {
            ...
            if (preg_match(
                '/' . str_replace('/', '\/', str_replace('\/', '/', $deviceRegex->regex)) . '/i', $uaString, $matches
            )
            ) {
                ...
                // device name
                $device->family = isset($deviceRegex->device_replacement) ? str_replace(
                    '$1', str_replace("_", " ", $matches[1]), $deviceRegex->device_replacement
