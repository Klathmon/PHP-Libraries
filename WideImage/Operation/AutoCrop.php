    ...
    function execute($img, $margin, $rgb_threshold, $pixel_cutoff, $base_color)
    {
        ...
        for ($y = 0; $y <= $cut_rect['bottom']; $y++) {
            ...
            for ($x = 0; $x <= $cut_rect['right']; $x++) {
                ...
                $diff = abs($rgb['red'] - $rgb_base['red']) + abs($rgb['green'] - $rgb_base['green']) + abs(
                        $rgb['blue'] - $rgb_base['blue']
                    );
        ...
        return $img->crop(
            $cut_rect['left'], $cut_rect['top'], $cut_rect['right'] - $cut_rect['left'] + 1,
            $cut_rect['bottom'] - $cut_rect['top'] + 1
        );
    }
}
